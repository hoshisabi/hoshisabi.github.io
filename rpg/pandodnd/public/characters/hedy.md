---
campaign_url: /rpg/pandodnd/public/
campaign_name: PandoDnD Pub Crawl
layout: character
title: Hedy
player: Gon
race: Firbolg
class: Firbolg Artificer 1 / Diviner Wizard 9
alignment: Chaotic Good
dnd_beyond: https://www.dndbeyond.com/characters/142336541
image: /rpg/pandodnd/public/images/portraits/hedy.jpeg
---

## Appearances

- **2026-05-27** — [Session 3 (May 27, 2026)](../sessions/2026-05-27) (*Reign of Frogs*)
- **2026-06-03** — [Session 4 (June 3, 2026)](../sessions/2026-06-03) (*Rock Bottom*)

_Note: drop-in roster; this page grows when the character appears in recaps._

## From D&D Beyond export

Snapshot from a Foundry VTT export (not guaranteed current on DDB): abilities **STR** 10 **DEX** 14 **CON** 14 **INT** 20 **WIS** 10 **CHA** 10 — **HP** 84. Subclass: **Diviner**. Background: Wandering Wizard. Companion: owl familiar named Alley.
