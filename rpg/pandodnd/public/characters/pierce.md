---
campaign_url: /rpg/pandodnd/public/
campaign_name: PandoDnD Pub Crawl
layout: character
title: Pierce
player: Mike
race: Human
class: Human Twilight Cleric 10
alignment: True Neutral
dnd_beyond: https://www.dndbeyond.com/characters/157952824
image: https://www.dndbeyond.com/avatars/43940/780/638607457552642121.jpeg
---

## Appearances

- **2026-05-13** — [Session 1 (May 13, 2026)](../sessions/2026-05-13) (*Unremembered Things*)

_Note: drop-in roster; this page grows when the character appears in recaps._

## From D&D Beyond export

Snapshot from a Foundry VTT export (not guaranteed current on DDB): abilities **STR** 10 **DEX** 14 **CON** 14 **INT** 12 **WIS** 18 **CHA** 10 — **HP** 93 / 93. Subclass on sheet: Twilight Domain *(Tasha’s).*
